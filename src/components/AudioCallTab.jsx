...
const GROQ_CHAT_URL     = "/openai/v1/chat/completions";           // via proxy
const GROQ_STT_URL      = "/openai/v1/audio/transcriptions";       // via proxy
const FLOWISE_URL       = "https://cloud.flowiseai.com/api/v1/prediction/c377ebd7-e572-4cd8-b520-ec6d9c45f8b4";
...
export default function AudioCallTab({ topic, language }) {
  ...
  async function startCall() {
    ...
    try {
      ...
      setAriaStatus("idle");
      startMonitorLoop();
      speakAria("Hi! I am Aria. Let us practice " + topic + " in " + (language?.name||"") + ". Just start speaking anytime — even while I am talking!");
    } catch(err) {
  ...
  function startMonitorLoop() {
    ...
    function tick() {
      ...
      // LISTENING: cumulative voice frames, auto-submit on silence
      ...
      // IDLE: wait for sustained voice before recording
      ...
      // SPEAKING: interrupt Aria if sustained user voice detected
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        if (!chunks.length || !streamRef.current) {
          ariaStatusRef.current = "idle"; setAriaStatus("idle"); return;
        }
      ...
      const res = await fetch(GROQ_CHAT_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are Aria, a friendly language tutor. Topic: "+topic+". Student learning: "+(language?.name||"")+". Reply in 1-2 sentences in "+(language?.name||"")+" with English in [brackets]. Correct mistakes gently."
            },
            ...history,
